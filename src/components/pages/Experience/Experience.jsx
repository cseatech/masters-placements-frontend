          ...
          <div className={styles.search_container}>
            ...
            <div className={styles.types}>
              {types.map((type, index) => (
                <div className={`rounded-[10px] px-5 py-1 w-auto items-center ${type.title === selectedType ? 'bg-[#895c95] text-white' : 'bg-[#292d33] text-white font-2 border-2 border-white hover:bg-[#400051]'}`} key={index}>
                  <button onClick={() => setSelectedType(type.title)}>{type.title}</button>
                </div>
              ))}
            </div>
          </div>
